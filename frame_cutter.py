...
def prepare_directories(base_dir):
    ...
    dirs = {
        'train': {
            ...
            'visualization': os.path.join(base_dir, 'train', 'visualization')
        },
        'test': {
            'images': os.path.join(base_dir, 'test', 'images'),
            'labels': os.path.join(base_dir, 'test', 'labels'),
            'visualization': os.path.join(base_dir, 'test', 'visualization')
        }
    }
...
def process_videos(videos_dir, base_dir="yolo_dataset", target_frames=300):
    ...
    for video_file in video_files:
        ...
        if class_name in existing_classes:
            print(f"\n클래스 '{class_name}'는 이미 존재합니다. 건너뜁니다.")
            continue
        
        print(f"\n=== 처리 중: {video_file} (클래스: {class_name}) ===")
        video_path = os.path.join(videos_dir, video_file)
        
        try:
            process_single_video(video_path, class_name, base_dir, target_frames)
            save_class_info(base_dir, class_name)
...
def save_frame_result(frame_data, frame_name, base_dir, class_name, split, dirs):
    """프레임 결과 저장"""
    img_path = os.path.join(dirs[split]['images'], f"{frame_name}.jpg")
    label_path = os.path.join(dirs[split]['labels'], f"{frame_name}.txt")
    vis_path = os.path.join(dirs[split]['visualization'], f"{frame_name}.jpg")
...
if __name__ == "__main__":
    ...
    base_dir = "yolo_dataset"  # 기본 데이터셋 디렉토리
